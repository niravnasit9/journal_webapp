...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Fetch role or create user doc if missing (Auto-heal for older accounts without names)
      ...
      if (userDocSnap.exists()) {
        ...
        // Auto-heal: create the missing user document (extract name from email prefix)
        const nameFallback = user.displayName || user.email?.split('@')[0] || "Unknown User";
      ...
      document.cookie = `userRole=${role}; path=/; max-age=86400`; // 1 day expiration
      ...
      setError(err.message || "Failed to login or register");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#fafafa] dark:bg-[#0a0f1c] text-gray-900 dark:text-white relative overflow-hidden font-sans transition-colors duration-500 px-4">
      ...
      <div className="absolute top-[-20%] left-[-10%] w-[50vw] h-[50vw] bg-indigo-600/5 dark:bg-indigo-600/10 rounded-full blur-[120px] pointer-events-none"></div>
      <div className="absolute bottom-[-20%] right-[-10%] w-[50vw] h-[50vw] bg-rose-600/5 dark:bg-rose-600/10 rounded-full blur-[120px] pointer-events-none"></div>
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between pl-1 pr-1">
              ...
              <Link href="#" className="text-[11px] font-bold text-indigo-500 hover:text-indigo-600 dark:text-indigo-400 transition-colors">Forgot?</Link>
            </div>
            <div className="relative group">
          ...
          >
            {loading ? (
              <>
                <i className="las la-spinner la-spin text-xl"></i> Authenticating...
              </>
            ) : (
              <>
                Sign In
