...
export const PsychologyDashboard: React.FC<{ trades: TradeDoc[], currency: "USD" | "INR" }> = ({ trades, currency }) => {
  ...
  if (trades.length === 0) {
    return (
      <div className="bg-[#0a0a0a] border border-neutral-800 rounded-2xl p-6 shadow-xl w-full text-center">
        ...
        <p className="text-sm text-neutral-400">Log your first trade to unlock deep behavioral insights and the Cost of Mistakes ledger.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Discipline Index */}
        <div className="bg-[#0a0a0a] border border-neutral-800 p-6 rounded-2xl flex flex-col items-center justify-center text-center shadow-xl">
          <h3 className="text-sm font-bold text-neutral-400 uppercase tracking-widest mb-6">Discipline Index</h3>
          ...
          <div className="relative w-40 h-40 flex items-center justify-center mb-4">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              ...
              <span className="text-xs text-neutral-500 font-bold uppercase">/ 100</span>
            </div>
          </div>
        ...
        <div className="bg-[#0a0a0a] border border-neutral-800 p-6 rounded-2xl lg:col-span-2 shadow-xl flex flex-col justify-between">
          <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-[#0a0a0a] border border-neutral-800 p-6 rounded-2xl shadow-xl">
          ...
          <ul className="space-y-4">
            {prescriptions.map((px, i) => (
              <li key={i} className="flex items-start gap-3 bg-[#121212] p-4 rounded-xl border border-neutral-800">
        ...
        {/* Emotion Table */}
        <div className="bg-[#0a0a0a] border border-neutral-800 p-6 rounded-2xl shadow-xl overflow-hidden flex flex-col">
          <h3 className="text-sm font-bold text-neutral-400 uppercase tracking-widest mb-6">Emotion vs. Performance</h3>
            ...
            <table className="w-full text-left border-collapse whitespace-nowrap">
              ...
              <tbody className="text-sm">
                {Object.keys(emotionStats).length === 0 ? (
                  <tr>
                    <td colSpan={4} className="px-4 py-8 text-center text-neutral-500">No emotional data tagged yet.</td>
                  </tr>
                ) : (
                  Object.entries(emotionStats).sort((a,b) => b[1].count - a[1].count).map(([emo, data]) => (
                    <tr key={emo} className="border-b border-neutral-800/50 hover:bg-[#121212] transition-colors">
                      <td className="px-4 py-3 font-bold text-white">{emo}</td>
                      <td className="px-4 py-3 text-neutral-400">{data.count}</td>
                      <td className="px-4 py-3 text-neutral-400">{((data.wins / data.count) * 100).toFixed(0)}%</td>
