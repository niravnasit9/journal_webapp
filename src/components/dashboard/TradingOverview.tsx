    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="premium-inner-box p-4">
          <p className="text-xs text-neutral-500 uppercase font-bold">Win Rate</p>
          <p className="text-xl font-bold text-white mt-1">{winRate.toFixed(1)}%</p>
        </div>
        <div className="premium-inner-box p-4">
          <p className="text-xs text-neutral-500 uppercase font-bold">Total Trades</p>
          <p className="text-xl font-bold text-white mt-1">{totalTrades}</p>
        </div>
        <div className="premium-inner-box p-4">
          <p className="text-xs text-neutral-500 uppercase font-bold">{isDomestic ? "Total Quantity" : "Total Lots"}</p>
          <p className="text-xl font-bold text-white mt-1">{totalVolume.toLocaleString()}</p>
        </div>
        <div className="premium-inner-box p-4">
          <p className="text-xs text-neutral-500 uppercase font-bold">{isDomestic ? "Total Points" : "Total Pips"}</p>
        ...
        <div className="premium-inner-box border border-orange-500/20 bg-orange-500/5 p-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-[#1a1a1a] p-3 rounded-lg border border-neutral-800">
              <p className="text-[10px] text-neutral-500 uppercase font-bold mb-1">Total Brokerage</p>
              <p className="text-sm font-bold text-white">₹{trades.reduce((s, t) => s + (t.tax_breakdown?.brokerage || 0), 0).toLocaleString()}</p>
            </div>
            <div className="bg-[#1a1a1a] p-3 rounded-lg border border-neutral-800">
              <p className="text-[10px] text-neutral-500 uppercase font-bold mb-1">Total STT</p>
              <p className="text-sm font-bold text-white">₹{trades.reduce((s, t) => s + (t.tax_breakdown?.stt || 0), 0).toLocaleString()}</p>
            </div>
            <div className="bg-[#1a1a1a] p-3 rounded-lg border border-neutral-800">
              <p className="text-[10px] text-neutral-500 uppercase font-bold mb-1">Total GST</p>
