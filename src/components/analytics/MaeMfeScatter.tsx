  ...
  if (trades.length < 5) {
    return (
      <div className="bg-[#0a0a0a] border border-neutral-800 rounded-2xl p-6 shadow-xl w-full h-[400px] flex flex-col items-center justify-center text-center">
        ...
        <p className="text-sm text-neutral-500 mt-2">Log at least 5 trades with MFE/MAE data to unlock this scatter plot.</p>
      </div>
    );
  }
      ...
      return (
        <div className="bg-[#121212] border border-neutral-800 p-3 rounded-lg shadow-xl">
          ...
          <div className="space-y-1 text-xs">
            ...
            <p className="text-neutral-400">Closed: {data.pnl >= 0 ? '+' : '-'}${Math.abs(data.pnl).toFixed(2)}</p>
          </div>
        </div>
      );
    }
